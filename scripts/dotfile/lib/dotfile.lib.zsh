...
function available_dotfiles() {
    ...
    if [ -z $group ]; then
        ...
        find "$SANDBIN_HOME/dotfiles" -maxdepth 2 -mindepth 2 -type f | sed "s/$curated_dotfiles_home//" | sed 's/\..*//'

    elif [ -d "$dotfiles_home$group" ]; then
        local curated_dotfiles_home=${dotfiles_home//\//\\\/}${group//\//\\\/}\\\/
        find "$SANDBIN_HOME/dotfiles/$group" -maxdepth 1 -mindepth 1 -type f | sed "s/$curated_dotfiles_home//" | sed 's/\..*//'
    else
