...
CONFIG_TEMPLATE_LOCATION="scripts/sandbin/sandbin.conf.template"

function generate_sandbin_config_file() {
    local sandbin_home=$1;
    local sandbin_template="$sandbin_home/$CONFIG_TEMPLATE_LOCATION"
    local sandbin_config="$sandbin_home/sandbin.conf"

    if [ ! -f "$sandbin_config" ]; then
        printf "${BOLD}Copying sandbin.conf file${NORMAL} template ('%s') to '%s' directory...\n" "$sandbin_template" "$sandbin_home"
...
function configure_sandbin_bootstrap() {
    ...
    if [ -f $config_file ]; then
        ...
        if grep -q "$START_MARK $sandbin_home_sha" "$config_file"; then
            echo "The ${BOLD}sandbin bootstrap is already configured${NORMAL} in '$config_file'"

        else
            if grep -q "$START_MARK" "$config_file"; then
                echo "${BOLD}Removing old sandbin configuration${NORMAL} from '$config_file' file"
                remove_between_marks_in_file "$START_MARK" "$END_MARK\\n" "$config_file"
            fi

            printf "%s\n%s\n%s\n" "$START_MARK $sandbin_home_sha" "source $sandbin_config" "$END_MARK" >> "$config_file"
            echo "${BOLD}The sandbin bootstrap '$sandbin_config' has been configured${NORMAL} in '$config_file'"
        fi
    fi
}
